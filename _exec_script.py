...
def html_to_pdf(html, output_filename, page_size='Letter', landscape=False, margin=None, full_bleed=False, expected_texts=None, banned_texts=None):
    """
    ...
    Args:
        ...
        expected_texts: OPTIONAL list of verbatim passages that MUST appear in
            the rendered PDF (edited answers, key headings, changed lines).
            The host's post-render QC reads the PDF back and FAILS the render
            report if any is missing.
        banned_texts: OPTIONAL list of passages this render was supposed to
            REMOVE or replace (the old answers, superseded lines). QC fails
            the render report if any still appears. When re-rendering an
            edited document (worksheets, answer keys, corrected pages), pass
            BOTH lists — the change is then verified in the rendered file
            instead of assumed from the code that made it.
    ...
    # Verification needles ride the request only when given: the host QC
    # treats their absence as not_checked, never as a pass. A bare string is
    # normalized to a one-element list — iterating it would explode into
    # per-character needles, which pass every filter and make the check
    # vacuously green (expected) or falsely red (banned).
    if isinstance(expected_texts, str):
        expected_texts = [expected_texts]
    if isinstance(banned_texts, str):
        banned_texts = [banned_texts]
    if expected_texts:
        request["expected_texts"] = [str(t) for t in expected_texts if str(t).strip()]
    if banned_texts:
        request["banned_texts"] = [str(t) for t in banned_texts if str(t).strip()]
    # File-backed request (robust against stdout truncation); marker carries path.
...
def render_document(spec, output_filename, theme="workbook"):
    ...
    Args:
        spec: dict {"title", "subtitle", "author", "blocks": [ {...}, ... ]}.
            ...
            tracker_grid, declaration, image, picture_page.
            picture_page (illustrated/kids books): binds MODEL-LETTERED pages
            (pass the lettered art as "image" with NO text field — the
            preferred pipeline), or typesets as the FALLBACK lane — {"type":
            "picture_page", "image": "art.png", "protect": [[x0,y0,x1,y1], ...]
            art-image-fraction boxes, then "text": "<verbatim caption>" OR
            "texts": [{"text", "near", "emphasis": whisper|quiet|normal|loud|
            shout}, ...] (spans in reading order; "near" pulls a span toward
            its subject). Typeset text renders exactly as written; unfittable
            pages fail the render with a per-page error list.
        output_filename: Base filename (without extension).
        theme: "workbook" | "journal" | "report" | "brochure" |
            "picture_book" (6x9in trim for picture_page books).
...
_USER_SOURCE = '\nimport os, shutil, zipfile, hashlib\n\nR = os.path.abspath("_build/repo")\n\n# Surface the two headline design artifacts at workspace root so they are delivered directly.\nshutil.copy(os.path.join(R, "docs/design/UI_DESIGN_SYSTEM.md"), "UI_DESIGN_SYSTEM.md")\nshutil.copy(os.path.join(R, "web/styles/mios.css"), "mios.css")\n\nfor f in ("UI_DESIGN_SYSTEM.md", "mios.css"):\n    print(f, os.path.getsize(f), "bytes")\n\n# Confirm the archive contents include the full operator + design layer\nwith zipfile.ZipFile("market_intelligence_os_full_build_v3.zip") as z:\n    names = z.namelist()\nkey = [n for n in names if any(k in n for k in\n       ("mios.css", "UI_DESIGN_SYSTEM", "console", "ops/", "deploy/", "mios.py",\n        "migrate_postgres", "OPERATIONS", "test_phase12"))]\nprint("\\nkey files inside the archive:")\nfor k in sorted(key):\n    print("  ", k)\nprint("\\ntotal entries in archive:", len(names))\n'
try:
...
try:
    ...
    for _root, _dirs, _fnames in os.walk("/workspace"):
        # `.stage-*` is the host's staging area for copying this workspace in.
        # A pass that aborted can leave one behind, still being written by
        # abandoned threads. Listing it would flatten half-written files onto
        # their real names on the host and publish them as deliverables.
        _dirs[:] = [
            _d for _d in _dirs
            if _d not in _skip_dirs and not _d.startswith(".stage-")
        ]
        for _fn in _fnames:
            if _fn.startswith("_"):
                continue
